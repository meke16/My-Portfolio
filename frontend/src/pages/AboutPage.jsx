...
function AboutPage() {
  ...
  const firstName = info?.name?.split(" ")[0] || "I";
  const focusCount = Array.isArray(about?.focusAreas) ? about.focusAreas.length : 0;
  const principleCount = Array.isArray(about?.principles) ? about.principles.length : 0;
  const journeyCount = Array.isArray(about?.journey) ? about.journey.length : 0;

  return (
    <section className="min-h-screen pt-16 pb-14 bg-[#0a0a0a] relative overflow-hidden">
      {/* Subtle grid */}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mb-8 max-w-3xl">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-white leading-tight">
              {about?.headline || `Who is ${firstName}?`}
            </h1>
            <div className="mt-3 w-10 h-0.5 bg-[#ff4500]" />
            <p className="mt-4 text-[#999] text-base md:text-[15px] leading-relaxed max-w-2xl">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
            ...
            <div className="rounded-xl border border-white/[0.07] bg-[#111] p-5">
              <p className="text-xs text-[#555] font-mono uppercase tracking-wider mb-1">Focus areas</p>
              <p className="text-white font-semibold">{String(focusCount).padStart(2, "0")}</p>
            </div>
            <div className="rounded-xl border border-white/[0.07] bg-[#111] p-5">
              <p className="text-xs text-[#555] font-mono uppercase tracking-wider mb-1">Principles</p>
              <p className="text-white font-semibold">{String(principleCount + journeyCount).padStart(2, "0")}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            <Link to="/projects"
              className="px-5 py-2.5 bg-[#ff4500] text-white text-sm font-semibold rounded-md hover:bg-[#cc3700] transition-colors shadow-[0_10px_25px_rgba(255,69,0,0.18)]">
              See featured work
            </Link>
            <Link to="/contact"
