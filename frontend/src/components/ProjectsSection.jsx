...
export default function ProjectsSection({ projects }) {
  ...
  const allTechnologies = [
    ...new Set(
      projects.flatMap(
        (project) => project.technologies?.split(",").map((t) => t.trim()) || []
      )
    ),
  ];
  ...
  return ( // ✅ START OF MAIN RETURN WRAPPER
    <>
      <section
        id="projects"
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {filteredProjects.map((project, index) => {
              ...
              const previewImage =
                images[0] || "https://via.placeholder.com/400x250";
              ...
              return (
                <div
                  ...
                  className="group bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm rounded-2xl overflow-hidden shadow-xl border border-white/20 hover:shadow-2xl hover:cursor-pointer transition-all duration-500 transform hover:-translate-y-2"
              ...
              <div className="relative overflow-hidden">
                <img
                  src={previewImage}
                  alt={project.title}
                  className="w-full h-58 object-cover group-hover:scale-110 transition-transform duration-500 cursor-pointer"
                  onError={(e) =>
                  ...
                  onClick={() => {
                    const imgs = images.length ? images : [previewImage];
                    // console.log("image clicked, imgs:", imgs);
                    openModal(imgs);
                  }}
                />

                {/* Overlay used only for visual effect — ignore pointer events so image receives clicks */}
                <div className="absolute inset-0 bg-linear-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"></div>
              </div>
...
<button
  onClick={closeModal}
  className="absolute right-1 top-[-40px] bg-black/60 hover:bg-black/80 text-red-500 
             w-10 h-10 flex items-center justify-center rounded-full 
             text-5xl font-bold transition backdrop-blur-sm z-50"
>
  ✕
</button>
...
<img
  ...
  className="w-full max-h-[80vh] object-contain rounded-xl shadow-xl z-40"
/>
      ...
      {modalImages.length > 1 && (
        <>
          <button
            ...
            className="absolute left-[-20px] top-1/2 -translate-y-1/2 text-white text-8xl px-4 font-bold"
          >
            ‹
          </button>
          ...
          <button
            ...
            className="absolute right-[-20px] top-1/2 -translate-y-1/2 text-white text-8xl px-4 font-bold"
          >
            ›
          </button>
        </>
